...
$user_id = $_SESSION['id'];
$id = $_GET['id'];
$infoVal=$A->Query("SELECT * FROM subcategoryquestionsvalue WHERE id='$id'");
$infoValFetch=$A->FetchAssoc($infoVal);
?>
<html lang="en">
<head>
    <title>ویرایش جواب</title>
...
<!--                            </div>-->
                            ...
                            <div class="form-group container-fluid">
                                <label for="Answer">پاسخ جدید را وارد کنید</label>
                                <br>

                                <input type="text" class="form-control" id="Answer" value="<?php echo $infoValFetch['value']?>">
                            </div>
...
<script>
    function send() {
        ...
        Notiflix.Confirm.Show('ویرایش جواب', 'آیا مطمئن هستید؟',
            'بله', 'خیر',
            function () {// Yes button callback
                $.ajax({
                    url: "8-2upCatQuesVal.php",
                    data: {
                        user_id: user_id,
